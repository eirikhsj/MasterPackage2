...
rolling_mod = function(q, ERA_NWP = load('input_data/ERA_NWP.Rda'), predictors, model='reg', window = 60,
    ...
    if (predictors > 0){
        for (l in 1:predictors){
            incl_vars = c(incl_vars, c('NWP1', 'NWP2', 'NWP3')[l])
            formula = paste0(formula, ' + ', c('NWP1', 'NWP2', 'NWP3')[l])}
    }
    ...
    for (i in seq_along(init_days)){
        ...
        if(predictors > 0){
            betas[,'NWP1_coef' := NWP1]
            betas[,'NWP1' :=NULL]
            if (predictors ==2){
                betas[,'NWP2_coef' := NWP2]
                betas[,'NWP2' :=NULL]}
        }
